function sstx = makeCVSpeechStruct(dirName)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Makes structure as .mat file using only CV pairs for SpeechSearch that contains
...
mkdir('SimMats');
cd('SimMats');
matPath = [pwd,'/CVphoMat.mat'];
save(matPath,'sstx');

fprintf('Spectral clustering completed, saving struct as %s \n',matPath)

%Also save csvs of matrices
csvwrite([pwd,'/CVWAbs.csv'],WA);
csvwrite([pwd,'/CVWNRG.csv'],WF1);
csvwrite([pwd,'/CVWNRGEnt.csv'],WF2);
csvwrite([pwd,'/CVWSpec.csv'],WF3);
csvwrite([pwd,'/CVWSpecEnt.csv'],WF4);
