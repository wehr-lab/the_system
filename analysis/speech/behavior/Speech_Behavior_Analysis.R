...
keep_cols <- c("consonant","speaker","vowel","token","correct","gentype","step","session","date")
...
load_generalization <- function(spfiles=spfiles,keep_cols=keep_cols,minsesh=FALSE){
  gendat <- data.frame(setNames(replicate(10,numeric(0),simplify = F),append(keep_cols,"mouse")))
...
load_timeseries <- function(spfiles=spfiles,winsize=winsize,ci=FALSE,minsesh=FALSE){
  ...
  if(ci){
    stepdat <- data.frame(setNames(replicate(10,numeric(0),simplify = F),c("mouse","rm","cilo","cihi","trialnum","date","session","step","level","diff")))
  } else {
    stepdat <- data.frame(setNames(replicate(8,numeric(0),simplify = F),c("mouse","rm","trialnum","date","session","step","level","diff")))
  }
  ...
  for (f in spfiles){
    ...
    if(ci){
      temp.df <- data.frame(setNames(replicate(10,numeric(0),simplify = F),c("mouse","rm","cilo","cihi","trialnum","date","session","step","level","diff")))
    } else {
      temp.df <- data.frame(setNames(replicate(8,numeric(0),simplify = F),c("mouse","rm","trialnum","date","session","step","level","diff")))
    }
...
plot_ts_area <- function(ts){
  ...
  ggsave(paste(basedir,"ts_area_",as.numeric(as.POSIXct(Sys.time())),".png"),plot=p.area,device="png",width=9,height=7,units="in",dpi=700)
