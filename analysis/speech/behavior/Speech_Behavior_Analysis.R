...
spfiles <- c("~/Documents/fixSpeechData/6924.csv",
             ...
             "~/Documents/fixSpeechData/7118.csv",
             "~/Documents/fixSpeechData/7120.csv")
...
source("/Users/Jonny/Github/the_system/analysis/speech/utils/speech_loading.R")

gendat <- load_generalization(spfiles=spfiles,keep_cols=keep_cols)
...
ts <- load_timeseries(spfiles="~/Documents/fixSpeechData/7120.csv",winsize=200)

ts <- ts[ts$step==15,]
ts.dec <- ts[seq(1,nrow(ts),10),]
ts_basic <- ggplot(ts.dec[ts.dec$session>=89 & ts.dec$session<=92,],aes(x=trialnum,y=rm))+
  geom_line(aes(colour=as.factor(session)))
ts_basic
